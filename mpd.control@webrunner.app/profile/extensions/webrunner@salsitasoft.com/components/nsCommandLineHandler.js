//@line 2 "/home/hudson/workdir/mozilla/trunk_src/extensions/webrunner/components/src/nsCommandLineHandler.js"
/* ***** BEGIN LICENSE BLOCK *****
 * Version: MPL 1.1
 *
...
WebRunnerCommandLineHandler.prototype = {
  ...
  handle : function(aCmdLine) {
    ...
    if (!aCmdLine.handleFlag("webrunner", false)) {
      // Not running from the stub so we're done
      return;
    }
    ...
    var webapp;
    if (environment.exists("WEBRUNNER_WEBAPP")) {
      webapp = environment.get("WEBRUNNER_WEBAPP");
    }
    else {
      webapp = aCmdLine.handleFlagWithParam("webapp", false);
    }

//@line 113 "/home/hudson/workdir/mozilla/trunk_src/extensions/webrunner/components/src/nsCommandLineHandler.js"
    
    if (webapp) {
      WebAppProperties.isWebApp = true;
      ...
      // We're running as normal Firefox so we're done
      return;
    }
    ...
    if (protocolURI.length > 0) {
      ...
      if (url) {
        ...
        while(enumerator.hasMoreElements()) {  
          ...
          if (helpwin.document.getElementById("content").currentURI.spec == newURI.spec) {
            win = helpwin;
            break;
          }
        }
    ...
    if (win) {
      if (protocolURI || url) {
        win.document.getElementById("content").loadURI(WebAppProperties.uri, null, null);
      }
      aCmdLine.preventDefault = true;
      return;
    }
...
var NSGetFactory = XPCOMUtils.generateNSGetFactory([WebRunnerCommandLineHandler]);
