//@line 2 "/home/hudson/workdir/mozilla/trunk_src/extensions/webrunner/components/src/nsPlatformGlue.js"
/*
//@line 41 "/home/hudson/workdir/mozilla/trunk_src/extensions/webrunner/components/src/nsPlatformGlue.js"
*/
...
const WEBRUNNER_PROTOCOL_PREFIX = "webrunner.protocol.";
...
// @007 Does it need to be a component?
...
PlatformGlue.prototype = {
  classDescription: "WebRunner platform API",
  ...
  registerProtocolFactory : function(uriScheme) {
    ...
    // Make sure we unregister any existing scheme
    if (protocolFactory) {
      registrar.unregisterFactory(PROTOCOL_HANDLER_CID, protocolFactory);
    }
    var contractId = "@mozilla.org/network/protocol;1?name=" + uriScheme;
    protocolFactory = MakeProtocolHandlerFactory(contractId, this);
    ...
    var alertListener = (!aListener) ? null :
      {
        observe: function(subject, topic, data) {
            if ((topic === "alertclickcallback") && !!textClickable) {
              ...
              /* Workaround to allow optional callback. Checking for null is no
  ...
  icon: function icon() {
    ...
    if (!this._icon) {
      ...
      this._icon = desktop.applicationIcon;
    }
    return this._icon;
  },
  ...
  registerProtocolHandler: function registerProtocol(uriScheme, uriString) {
    ...
    if (uriString != null) {
        this._prefs.setCharPref(WEBRUNNER_PROTOCOL_PREFIX + uriScheme, uriString);
    }
  },
  ...
  unregisterProtocolHandler: function unregisterProtocol(uriScheme) {
    ...
    // And remove the pref
    this._prefs.clearUserPref(WEBRUNNER_PROTOCOL_PREFIX + uriScheme);
  ...
  getProtocolURI : function getProtocolURI(uriSpec, callback) {
    ...
    try {
      var uriString = this._prefs.getCharPref(WEBRUNNER_PROTOCOL_PREFIX + uriScheme);
...
var NSGetFactory = XPCOMUtils.generateNSGetFactory([PlatformGlue]);
