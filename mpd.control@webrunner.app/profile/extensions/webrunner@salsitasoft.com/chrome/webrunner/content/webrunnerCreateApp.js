...
Components.utils.import("resource://webrunner/modules/WebAppInstall.jsm", WebRunner);
Components.utils.import("resource://webrunner/modules/WebAppProperties.jsm", WebRunner);
Components.utils.import("resource://webrunner/modules/FileIO.jsm", WebRunner);

WebRunner.CreateApp = {
  convertToApplication : function(useDocumentHref) {
    if (useDocumentHref) {
      WebRunner.WebAppProperties.uri = gBrowser.contentDocument.location.href;
      var regexp = /^[\w ]*/;
      ...
      WebRunner.WebAppProperties.name = title;
      WebRunner.WebAppProperties.appBundle = null;
    }

    window.openDialog("chrome://webrunner/content/install-shortcut.xul", "install", "centerscreen,modal", WebRunner.WebAppProperties, this.exportPrefs);
  },
  ...
  exportPrefs : function()
  {
    ...
    var prefList = prefs.getChildList("", {});
    ...
    var file = WebRunner.WebAppProperties.getAppRoot();
    file.append("prefs.json");
    WebRunner.FileIO.stringToFile(json, file);
  },

  onLoad : function() {
    var uriloader = Cc["@mozilla.org/uriloader;1"].getService(Ci.nsIURILoader);
    uriloader.registerContentListener(WebRunner.ContentListener);

    gBrowser.addProgressListener(WebRunner.WebListener);
  }
}

WebRunner.WebAppDownload = {
  targetFile : null,
  ...
  install : function(file) {
    ...
    var packageDir = WebRunner.WebAppInstall.install(file);
    WebRunner.WebAppProperties.init(packageDir);
    WebRunner.CreateApp.convertToApplication(false);
    WebRunner.CreateApp.exportPrefs();
  },
...
WebRunner.WebApps = function(theDocument)
{
  this.webappCount = 0;
  this.strings = document.getElementById("WebRunner_strings");
  theDocument.addEventListener("DOMLinkAdded", this, false);
  theDocument.addEventListener("DOMContentLoaded", this, false);
}

WebRunner.WebApps.prototype = {
  ...
  onContentLoaded : function(event) {
    ...
    if (this.webappCount > 0 && notificationbox.getNotificationWithValue("WebRunner") == null) {
      document.getElementById("WebRunnerNotificationPopup").addEventListener("popupshowing", this, false);
      notificationbox.appendNotification(
        this.strings.getString("WebRunnerNotificationText"),
        "WebRunner",
        "chrome://webrunner/skin/webrunner48.png",
        notificationbox.PRIORITY_INFO_LOW,
        [ { accessKey: null,
            callback: null,
            label: this.strings.getString("WebRunnerNotificationButton"),
            popup: "WebRunnerNotificationPopup" }
        ]
      );
    }
  },
  ...
  fillPopupList : function(event)
  {
    ...
    for (var i=0; i<links.length; i++) {
      if (links[i].rel == "webapp") {
        ...
        var label = this.strings.getFormattedString("WebRunnerNotificationMenu", [ links[i].getAttribute("title") ]);
  ...
  installWebApp : function(event)
  {
    ...
    WebRunner.WebAppDownload.download(webappUri);
  }
}

WebRunner.WebListener = {
  QueryInterface: function(iid) {
  ...
  onLocationChange : function (webProgress, request, loc) {
    ...
    if (request) {
      new WebRunner.WebApps(webProgress.DOMWindow.document);
    }
  },
...
WebRunner.ContentListener = {
  ...
  doContent: function(contentType, isContentPreferred, request, contentHandler) {
    ...
    var targetFile = WebRunner.WebAppDownload.getTemporaryFileForUri(webappUri);

    downloader.init(WebRunner.WebAppDownload, targetFile);
...
window.addEventListener("load", WebRunner.CreateApp.onLoad, false);
