...
FaviconDownloader.prototype = {
  ...
  onContentLoaded : function(event)
  {
    if (!this._mimeType)
    {
      ...
      this.loadIcon(null, event.originalTarget.baseURIObject.prePath + "/favicon.ico");
    }
  },
  ...
  onStopRequest : function(request, context, statusCode)
  {
    ...
    if (statusCode == Components.results.NS_OK &&
      ...
      // Make sure we did't download an empty favicon
      if (this._storageStream.length > 0) {
        this._haveIcon = true;

        if (!this._mimeType) {
          this._sniffMimeType(request);
        }

        // Need to have the right MIME type to convert ICO files
        if (this._mimeType == "image/ico" || this._mimeType == "image/x-icon") {
          this._mimeType = "image/vnd.microsoft.icon";
        }
      }
  ...
  _sniffMimeType : function(request) {
    var categoryManager = Cc["@mozilla.org/categorymanager;1"].getService(Ci.nsICategoryManager);
    var sniffers = categoryManager.enumerateCategory("content-sniffing-services");
    while (!this._mimeType && sniffers.hasMoreElements()) {
      var snifferContractID = sniffers.getNext().QueryInterface(Ci.nsISupportsCString);
      var sniffer = Cc[snifferContractID.data].getService(Ci.nsIContentSniffer);
      var bis = Cc["@mozilla.org/binaryinputstream;1"].createInstance(Ci.nsIBinaryInputStream);
      bis.setInputStream(this.imageStream);
      var length = bis.available();
      this._mimeType = sniffer.getMIMETypeFromContent(request, bis.readByteArray(length), length);
    }
  }
};
